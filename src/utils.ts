...
export async function fetchAppById(appId: string, appBackendUrl: string): Promise<string> {
  if (appNameCache[appId]) {
    return appNameCache[appId];
  }
  try {
    const response = await fetch(`${appBackendUrl}/api/zkfetch/sdk/${appId}`);
...
export async function sendLogs(
  ...
  try {
    const getAppName = await fetchAppById(applicationId, appBackendUrl);
